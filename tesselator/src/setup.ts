...
const loadConfig = (filePath: string): any => {
    try {
        ...
        throw new Error(
            `Failed to load config file at ${filePath}: ${error.message}`,
        );
    }
};
 ...
 */
const installDependencies = (directoryPath: string) => {
    if (fs.existsSync(directoryPath)) {
        ...
        try {
            ...
            throw new Error(
                `Failed to install dependencies in ${directoryPath}: ${error.message}`,
            );
        }
    } else {
        console.warn(
            `Directory not found: ${directoryPath}. Please run 'npm install' manually.`,
        );
    }
};
