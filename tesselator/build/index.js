...
const teseractPlugin = JSON5.parse(readFileSync(path.join(resourcesPath, "teseract.plugin.json")).toString());
const BPManifest = JSON5.parse(readFileSync(path.join(resourcesPath, "/behavior/manifest.json")).toString());
for (const dependency of teseractPlugin.modules) {
    ...
    if (BPManifest.dependencies.find((dep) => dep.module_name == name)) {
        console.warn(`Module ${name}@${version} is already present in the manifest, and won't be overriden`);
    }
    else {
...
const RPManifest = JSON5.parse(readFileSync(path.join(resourcesPath, "/assets/manifest.json")).toString());
BPManifest.dependencies.unshift({
...
if (scriptModule) {
    console.warn(`Scripting module is already present in the manifest, and won't be overriden. WARNING: This may cause entrypoint file to differ from entry point auto-generated by sculk-loom.`);
}
else {
...
writeFileSync(path.join(regolithTmp, "/RP/manifest.json"), JSON.stringify(RPManifest, null, 5));
writeFileSync(path.join(regolithTmp, "/BP/manifest.json"), JSON.stringify(BPManifest, null, 5));
class Tesselator {
    static writeIndex() {
  ...
  return c > 3 && r && Object.defineProperty(target, key, r), r;
    ...
    static readPluginData() {
        const { entryPoints, name, id, description, version, external, authors, } = teseractPlugin;
